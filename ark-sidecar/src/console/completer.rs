...
pub(crate) struct LspCompleter {
    /// Shared handle to the LSP client.
...
impl LspCompleter {
    pub fn new(client: Arc<LspClient>, runtime_handle: tokio::runtime::Handle) -> Self {
        Self {
            client,
            runtime_handle,
        }
...
impl Completer for LspCompleter {
    fn complete(&mut self, line: &str, pos: usize) -> Vec<Suggestion> {
        ...
        match self.runtime_handle.block_on(async {
            tokio::time::timeout(LSP_COMPLETION_TIMEOUT, client.complete(&buffer, pos)).await
        }) {
            Ok(Ok(response)) => {
